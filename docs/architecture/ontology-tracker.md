...
| `campaign.secrets` → AI layer | `types/Secret.ts` | `partial` | Fully live in the UI (SecretsTracker, backlinks, continuity id-universe) but **never reaches `contextBuilder.ts` or any `services/ai/*` module** — the gap E3 closes. |
