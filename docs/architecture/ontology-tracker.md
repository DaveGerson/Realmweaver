...
| E4 | `Plot.escalation?` — Front/threat-clock track (`doom`, ordered `steps[]` with `triggered` / `triggeredInSessionId`) | schema (optional field) | `types/Plot.ts` · `SessionEndWizard` · `services/ai/worldSimulation.ts` · purge sweep (`triggeredInSessionId`) · fully-escalated lint | `partial` — the minimal front shipped 2026-09-06 as `Plot.clock?` (`{ segments, filled }`) + `Plot.ifIgnored?` (one move, not an ordered `steps[]`; non-id-bearing, so no purge/remap cost), ticked from the Session Runner and the Plot editor, read by `contextBuilder`, Tonight's Table and the `clock-expired` info lint (`docs/design/unstructured-play.md` §3.3). The per-step `triggeredInSessionId` ledger and the world-simulator wiring remain not-implemented — the ledger is the part `lazy-dm-lens.md` §5 flags as anti-lazy. |
