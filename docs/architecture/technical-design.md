...
## AI Dungeon Master extension (SRD 5.2.1 preview)

The `ai-dm` view adds a versioned `Campaign.dungeonMaster` aggregate, pure rules transactions under `services/rules/`, local SRD retrieval, and structured proposal/outcome narration through the existing AI facade. `campaignService.commitDungeonMaster` applies a revision-checked, validated state update. Live AI DM state resets when a campaign is duplicated. Existing campaign entities and the legacy Combat Tracker remain separate.

New modules include `types/DungeonMaster.ts`, `services/ai/dungeonMaster.ts`, `services/dungeonMasterService.ts`, `services/rules/{engine,dice,srd,characters,characterRules,validation}.ts`, and `components/views/AiDungeonMaster.tsx`. The corpus is generated by `scripts/import_srd.py`. The full design, behavior boundaries and completion gates are documented in [AI Dungeon Master](../ai-dungeon-master.md). Complete SRD retrieval must not be described as complete mechanical automation.
