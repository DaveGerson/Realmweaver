    ...
    <div className="relative bg-slate-900/50 p-4 rounded-lg border border-slate-800 space-y-3">
      {isLoading && (
        <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm flex flex-col items-center justify-center rounded-lg z-10 transition-opacity duration-300 animate-in fade-in">
          ...
          <p className="mt-2 text-sm text-slate-300">Generating Item...</p>
        </div>
      )}
      <div className="flex items-center gap-2">
        <Icons.Sparkles className="w-5 h-5 text-indigo-400" />
        <h3 className="text-md font-semibold text-slate-200 font-serif">Generate New Item</h3>
      </div>
      <p className="text-sm text-slate-400">
        Describe a magical item. (e.g., "A sword that glows near goblins")
      </p>
      <textarea
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        placeholder="Enter item prompt here..."
        rows={3}
      ...
      <Button onClick={handleGenerate} disabled={isLoading} className="w-full">
        {isLoading ? (
          ...
          <>
            <Icons.Items className="w-4 h-4 mr-2" />
            Create Item
          </>
        )}
      </Button>
