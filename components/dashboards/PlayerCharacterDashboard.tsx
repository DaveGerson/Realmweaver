    ...
    <div className="p-6 md:p-8 h-full overflow-y-auto custom-scrollbar space-y-8 animate-fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <h2 className="text-2xl font-bold font-serif text-slate-200 mb-4">Player Characters ({(playerCharacters || []).length})</h2>
            ...
            {(playerCharacters || []).map(pc => {
              ...
              return (
                <button
                  ...
                  className="bg-slate-900/50 p-4 rounded-lg border border-slate-800 border-l-4 border-l-indigo-500 text-left hover:bg-slate-800 hover:border-slate-700 hover:border-l-indigo-400 transition-all space-y-2"
                >
                  <h3 className="font-semibold text-indigo-400 leading-tight">{pc.characterSocial.characterName}</h3>
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] bg-indigo-900/40 text-indigo-300 border border-indigo-500/30 rounded-full px-2 py-0.5">
                      {species}
                    </span>
