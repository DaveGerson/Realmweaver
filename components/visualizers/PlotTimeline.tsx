...
function computeWarnings(plots: Plot[], sessions: SessionLog[]): PlotWarning[] {
  ...
  for (const plot of plots) {
    ...
    // Look at the last 3 sessions ordered chronologically
...
export const PlotTimeline: React.FC<PlotTimelineProps> = ({
  ...
  const showTooltip = (e: React.MouseEvent, content: string) => {
    ...
    setTooltip({ visible: true, x: rect.left + rect.width / 2, y: rect.top - 8, content });
  };
  ...
  const COL_WIDTH = 120; // px per session column
  const ROW_HEIGHT = 40; // px per plot row
  const LABEL_WIDTH = 152; // px for fixed left label column

  // We render a fixed left column (plot names) next to a horizontally-scrollable
  // grid (session columns). They share the same vertical rhythm.
              ...
              {plots.map((plot, pi) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => onSelectPlot?.(plot.id)}
                  >
                    {/* Horizontal line spanning all columns — drawn as an absolutely
                        positioned element so it sits behind the status dots */}
                    <div
