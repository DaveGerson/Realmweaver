...
| `realmWeaver.ts` | Entity schemas (`npcSchema`…`pointOfInterestSchema`) + the shared `generateEntity` config-driven generator; `generateNpc/Location/Faction/Item/Scene/Adventure/Article/PoiFromLoot`; `generateSecretBatch` (R2 — ~10 `SecretDraft`s in one schema call; mock parity in `mockService.ts`); `locationSchema` also carries an optional `aspects: string[]` (Lazy DM step 5) generated alongside every new location and normalised in `postProcess` (`normalizeAspects`: trim, drop blanks, cap 4, tolerate absent), plus standalone `generateLocationAspects(location, campaignContext?)` for retrofitting an existing one. |
| `dmCoach.ts` | `generateNarration`, `generateImprovisation`, `generateCallbackComplication` (zero-prompt reincorporation; mock parity in `mockService.ts`), `generateRollableTable`, `generateEnhancedText`, `generateColdOpen` (P4 — ~150-word read-aloud previously-on from recap + loose ends + starred moments, styleProfile voice; mock parity), `generateStrongStart` (`StrongStartRequest` — the other two strong-start styles: `'action'` needs no past session, `'reincorporate'` requires a `dormantPiece`; `'previously-on'` stays on `generateColdOpen`; styleProfile voice; mock parity), `generateSceneMenu(campaignContext?)` (`SceneMenuDraft[]` — a fixed, zero-prompt request for 5–6 scene drafts for the lazy path's Beats step; mock parity), `generateGmIntrusion(campaignContext?, sceneSummary?)` (zero-prompt, zero-precondition live complication; mock parity), `generateExtras` (`ExtraNpc[]` — 4–6 name + one-detail throwaway NPCs; mock parity), `generateCheckInQuestions(request = {})` (`CheckInQuestionsRequest` `{ campaign?, campaignContext?, useLiteModel? }` — 4–5 between-session questions that fold in `PlayerCharacter.playerFlags` when present; mock parity), `generateSessionRecap` (`SessionRecapResult`; two parallel model calls — GM recap on full context, player recap on a `player-safe` context via `PlayerSafeRecapContext`, so unrevealed GM truth is structurally excluded), `analyzeSessionNotes` + their schemas. |
