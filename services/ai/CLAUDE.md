...
| `dmCoach.ts` | `generateNarration`, `generateImprovisation`, `generateCallbackComplication` (zero-prompt reincorporation; mock parity in `mockService.ts`), `generateRollableTable`, `generateEnhancedText`, `generateSessionRecap` (`SessionRecapResult`; two parallel model calls — GM recap on full context, player recap on a `player-safe` context via `PlayerSafeRecapContext`, so unrevealed GM truth is structurally excluded), `analyzeSessionNotes` + their schemas. |
